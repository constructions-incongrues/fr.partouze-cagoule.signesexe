...
deploy:
	rsync $(RSYNC_OPTIONS) -avz --exclude *.zip --exclude /src/.env --exclude /src/public/.htpasswd --exclude /src/public/database* --exclude /src/data/blacklist.txt --exclude /.git ./ -e 'ssh' partouze-cagoule@ftp.pastis-hosting.net:signesexe.partouze-cagoule.fr/ 
